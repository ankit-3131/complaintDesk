...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Passcode (OTP) sent to your email!');
      navigate('/reset-password');
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Failed to send OTP. Please check the email.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex flex-col items-center text-center">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 via-blue-600 to-cyan-500 p-[1px] shadow-lg shadow-indigo-500/30 flex items-center justify-center mb-3">
              <div className="w-full h-full bg-slate-950 rounded-[15px] flex items-center justify-center">
                <KeyOutlinedIcon className="text-cyan-400 !text-2xl" />
              </div>
            </div>
            ...
            <p className="text-xs sm:text-sm text-slate-400 mt-1.5 leading-relaxed">
              Enter your registered email and we'll dispatch a one-time verification passcode (OTP) to recover your account.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-300">
                Registered Email Address <span className="text-cyan-400">*</span>
              </label>
